...
This repository is a single plugin that bundles seven self-contained engineering and research skills. Each skill is a model-agnostic instruction set: it classifies the task, gathers evidence, applies domain references, and verifies before handoff.
...
- `skills/software-architecture-engineering-uap` — Design, evaluate, document, modernize, and implement software architecture: requirements, context, style selection, architecture views, NFR tactics, IAM/auth, security patterns, cost, fitness functions, and ADR/RFC work.
